namespace EasyButtons.Editor.Utils
{
    ...
    public static class EnumExtensions
    {
        ...
        public static void SetComponentMatching<T>(this GameObject anchor, out GameObject[] objecArray) where T : System.Enum 
        {
            var names = System.Enum.GetNames(typeof(T));  
            objecArray = new GameObject[names.Length]; 
            for(int i=0; i< names.Length;i++)
                objecArray[i] = anchor.transform.Find(names[i]).gameObject;
        }
    }
}